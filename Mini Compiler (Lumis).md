...
### **6.4 Execution Engine** {#code-generation}

Once a program passes semantic analysis, the compiler executes the program via its in-memory interpreter:
- **In-Memory AST Execution (`-r` / `--run` or default):** Interprets and executes Lumis program statements directly.
